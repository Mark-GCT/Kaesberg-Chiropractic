(() => {
	...
	const scrollManager = {
		handleScrollEffects() {
			...
			this.manageInertElements(isScrolled);
		},

		manageInertElements(isScrolled) {
			// Top bar always becomes inert on scroll
			if (elements.topBar) elements.topBar.inert = isScrolled;

			// Dark mode toggle always stays focusable
			if (elements.darkModeToggle) elements.darkModeToggle.inert = false;

			// Nav button: only focusable on desktop (hidden on mobile anyway)
			if (elements.navButton) {
				elements.navButton.inert = isMobile() ? true : false;
			}
		},
	};
